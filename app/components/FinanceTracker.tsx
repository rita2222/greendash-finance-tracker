...
export default function App(){
  ...
  useEffect(()=>{
    (async()=>{
      try{
        const i=await window.storage.get('ft_inv');if(i)setInvoices(JSON.parse(i.value))
        const e=await window.storage.get('ft_exp');if(e)setExpenses(JSON.parse(e.value))
        const g=await window.storage.get('ft_gexp');if(g)setGrantExps(JSON.parse(g.value))
        const fe=await window.storage.get('ft_future');if(fe)setFutureExpenses(JSON.parse(fe.value))
        const s=await window.storage.get('ft_ivasub');if(s)setIvaSubmitted(JSON.parse(s.value))
      }catch{}
      setLoaded(true)
    })()
  },[])
